...
<body>
	...
	<!--
	result2에 not을 붙여 반대의 조건을 만들어 else와 같은 구문이 된다.
	-->
...
<!----------------------------------------------------------------------------------------------------------->
	...
	<h4>연습문제 : if태그</h4>
    <!--  
    아이디, 패스워드를 입력후 submit버튼을 누르면 EL식을 통해 파라미터를 받은 후 
    tjoeun/1234 인 경우에는 'tjoeun님, 하이룽~'이라고 출력한다. 
	...
	<c:if test="${ not empty param.user }">
		...
		<!--
		아이디, 패스워드의 일치여부를 판단한 후 결과를 login에 저장한다.
		if태그는 else구문이 별도로 없으므로 not을 통해 else와 같은 구문을 구성해야한다.
		-->
		<c:if test="${ param.user eq 'tjoeun' and param.pass eq '1234' }" var="login">
			${ param.user }님, 하이룽~
		</c:if>
