...
<body>
	...
	: 외부의 문서를 현재문서에 포함시킨다.
	 ...
	 따라서 서로 다른 페이지므로 page영역은 공유되지 않고 request영역만 공유된다.
	 url속성에 절대경로로 지정할 경우 컨텍스트루트(JSPStudy) 경로명은 포함하지 않는다.
	-->
	
	<!-- request 영역에 속성을 저장한다. -->
	<c:set var="requestVar" value="MustHave" scope="request" />
	<!-- 
	var 속성을 미사용시: include 액션태그와 동일하게 현재 위치에 즉시 외부문서를 포함시킨다.
	var 속성을 사용시: var에 지정한 변수를 EL로 출력하는 곳에 초함된다.
	...
	만약 절대경로를 사용해야 한다면 하드코딩 하는것보다 request 내장객체에서 제공하는 메서드를 사용하는 것이 좋다.
	웹프로그래밍은 웹서버에 배포하는것이 목적이므로 서버의 환경이 달라지면 경로까지 수정해야하는 경우가 생기므로
	이를 최소화할 수 있도록 개발하는것이 좋다.
	특히 JSTL에서 URL을 지정할 때 컨텍스트루트 경로는 명시하지 않아도 되므로 일반적인 방식보다 작성에 유리한점이 있다.
	-->
	<h4>이미지 삽입하기</h4>
