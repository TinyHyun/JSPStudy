...
$(function () {
	...
	//만약 해당 페이지에 2개 이상의 ajax() 함수가 있다면 반복적인 설정하지 않아도 되므로 편리하다.
	$.ajaxSetup({
		url : "../NaverSearchAPI.do", //요청URL(ajax로 전송한 값이 전달되는 위치)
		type : "get", //전송방식
		contentType : "text/html;charset:utf-8;", 
		dataType : "json", //콜백 데이터의 형식
		success : sucFuncJson, //성공했을 때 실행 할 콜백 함수
		error : errFunc //실패했을 때 실행 할 콜백 함수
	});
	...
	//<select> 박스의 항목을 선택하는 순간 발생되는 change 이벤트에 대한 처리
...
function sucFuncJson(d) {
	...
	*/
	...
	//JS의 innerHTML()과 동일한 역할의 함수로 html 형식으로 삽입한다.
...
<body>
	<div class="container">
		...
		<div class="row">
			<!-- ajax를 통해 폼값을 전송할 때 jQuery에서 대부분의 설정을 하므로 <form> 태그는 아래와 같이 기본설정만 하면 된다. -->
			<form id="searchFrm">			
				한페이지에 20개씩 노출됨 <br />
				...
				<!-- jQuery를 통해 폼값을 전송하므로 버튼도 submit타입이 아닌 button타입으로 설정하면 되소, 해당 엘리먼트를 선택할 수 있도록 id속성을 부여해야한다. -->
