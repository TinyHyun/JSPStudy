...
public class BoardDAO extends JDBConnect {
	...
	public int selectCount(Map<String, Object> map) {
		...
		try {
			...
			//쿼리문 실행 후 결과는 ResultSet(rs)으로 반환한다.
	...
	/*
	작성된 게시물을 인출하여 반환한다.
	특히 반환값은 여러개의 레코드를 반환할 수 있고 순서를 보장해야 하므로 List 컬렉션을 사용한다. 
	*/
	public List<BoardDTO> selectList(Map<String, Object> map) {
		
		/*
		List계열의 컬렉션을 생성한다. -> List계열의 컬렉션 아무거나 사용해도 무관하다.
		...
		*/
		...
		최근 게시묾이 상단에 출력되야 하므로 일련번호의 내림차순으로 정렬한다.
		*/
		...
		try {
			...
			//2개 이상의 레코그가 반환될 수 있으므로 while문을 사용한다.
			while (rs.next()) {
		...
		//인출한 레모드를 저장한 List를 호출한 지점으로 반환한다.
		return bbs;
		
		
	}
	
	//게시물 입력을 위한 메서드. 폼값이 저장된 DTO 객체를 인수로 받는다.
	public int insertWrite(BoardDTO dto) {
		...
		try {
			/*
			인파라미터가 있는 동적 쿼리문으로 insert문을 작성한다.
	...
	public BoardDTO selectView(String num) {
		...
		try {
			...
			next() 메서드는 ResultSet으로 반환된 게시물을 홗인해서 존재하면 true를 반환해준다. 
			*/
			if (rs.next()) {
				...
				/*
				각 컬럼의 값을 추출할 때 1부터 시작하는 인덱스와 컬럼명 둘다 사용할 수 있다.
				날짜인 경우에는 getDate() 메서드로 추출할 수 있다.
				*/
				dto.setContent(rs.getString("content"));
				dto.setPostdate(rs.getDate("postdate"));
				dto.setId(rs.getString("id"));
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map) {
		...
		*/
		...
		try {
			//인파라미터가 있는 쿼리문으로 prepared 객체 생성
			psmt = con.prepareStatement(query);
			//인파라미터 설정
