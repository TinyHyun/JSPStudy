...
public class MVCBoardDAO extends DBConnPool {
    ...
    public int selectCount(Map<String, Object> map) {
        ...
        try {
        	//인파라미터가 없는 정적쿼리문이므로 statement 인터페이스 생성
    ...
    //목록에 출력 할 게시물을 얻어오기 위한 메서드
    public List<MVCBoardDTO> selectListPage(Map<String, Object> map) {
    	
    	//mvcboard 테이블을 대상으로 하므로 타입매개변수 확인 필요함.
        List<MVCBoardDTO> board = new Vector<MVCBoardDTO>();
        
        //페이징 처리를 위한 서브쿼리문. 특정 페이지에 해당하는 rownum의 구간으로 게시물을 인출한다.
    ...
    public int insertWrite(MVCBoardDTO dto) {
    	...
    	try {
    		/*
    		쿼리문의 일련번호는 모델1 게시판에서 생한 시퀀스를 그대로 사용한다.
    ...
    public boolean confirmPassword(String pass, String idx) {
    	...
    	try {
    		...
    		rs = psmt.executeQuery();
    		//count() 함수는 반드시 결과값이 있으므로 if문 없이 next()를 호툴
    		rs.next();
    		...
    		/*
    		쿼리문 실행 도중 예외가 발생되면 바로 catch절로 넘어오므로 이 경우에도 false로 값을 설정해야 한다.
    		*/
    		isCorr = false;
    		e.printStackTrace();
    	}
	...
	public int updatePost(MVCBoardDTO dto) {
		...
		try {
			...
			/*
			서블릿 게시판은 비회원제이므로 게시물 수정시 일련번호 뿐만 아니라 패스워드까지 조건 절로 추가한다.
