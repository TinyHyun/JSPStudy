...
: 디렉토리 서비스에서 제공하는 데이터 및 객체를 찾아서 참조(Lookup)하는 API로 쉽게 말하면
 외부에 있는 객체를 이름으로 찾아오기 위한 기술이다.
 
DBCP(DataBase Connection Pool: 커넥션풀)
: DB와 연결된 커넥션 객체를 미리 만들어 풀(Pool)에 저장해 뒀다가 필요할 때 가져다 쓰고 반납하는 기법을 말한다.
 DB에 부하를 줄이고 자원을 효율적으로 관리 할 수 있다. (워터파크의 유수풀과 동일하다.)
*/
public class DBConnPool {

    public Connection con;
    public Statement stmt; //정적쿼리문
    public PreparedStatement psmt;  //동적쿼리문
    ...
    */
    ...
    public DBConnPool() {
    	...
    	try {
    		//1. Context 객체를 생성한다. Tomcat웹서버라 생각하면 된다.
    ...
    즉 여기서의 close()는 객체의 소멸이 아닌 반납이다.
    */
    public void close() {
