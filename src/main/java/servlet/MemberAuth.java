...
public class MemberAuth extends HttpServlet {
	...
	/*
	클라이언트가 최초로 요청했을 때 서블릿 객체가 생성되는데
	이때 최소 1번만 호출되는 init() 메서드에서 DB연결 처리를 한다.
	*/
	@Override
	public void init() throws ServletException {
		...
		모델2 방식에서는 서블릿이 먼저 요청을 받기 때문에 모델1 방식과 같이 JSP에서 매개변수로 내장객체를 전달할 수 없다.
	...
	*/
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		*/
		...
		아이디, 패스워드를 통해 회원인증을 진행한 후 일치하는 정보가 있다면 회원레코드를 DTO에 저장 후 반환한다.
		*/
		...
		if (memberName != null) {
			...
			*/
			...
			/*
			회원인증에 실패한 경우에는 서블릿 초기화 파라미터와 비교해서 최고관리자인지 확인한다.
