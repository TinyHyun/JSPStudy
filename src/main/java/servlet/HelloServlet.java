...
4. request 내장객체를 사용할 때는 매개변수 req를 이용한다.
   response는 resp로 사용하면 된다.
*/
public class HelloServlet extends HttpServlet {
	
	//서블릿 작성시 발생되는 경고(Warning)를 위해 추가한다.
	private static final long serialVersionUID = 1L;  //없어도 실행에는 영향을 미치지 않는다.
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		/* request 영역은 forward된 페이지까지 공유되므로 Servlet에서 저장한 속성은 JSP에서 사용할 수 있다. */
	}
}
