...
public class NaverSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		String clientId = "Na1u71f0rhPL9LRfWGCt"; //애플리케이션 클라이언트 아이디
        String clientSecret = "LKVeGiRzUD"; //애플리케이션 클라이언트 시크릿
        
        //검색어는 파라미터로 받은 후 UTF-8로 인코딩 처리한다.
        ...
        /*
        네이버 검색 API의 요청 URL은 아래와 같이 정의되어 있다.
        우리는 JSON의 결과를 받을 것 이므로 첫번째 주소로 요청한다.
        파라미터는 query(검색어), display(출력 할 갯수), start(출력시 시작 위치) 3가지를 사용한다.
        */
	...
	//아래에 있는 3개의 메서드는 수정 없이 그대로 사용하면 된다.
